...
{
    public OVRGazePointer playerPointerRaycaster;
    ...
    {   
		if(_holding)
        {
            ...
            {
                Vector3 rayDir = playerPointerRaycaster.gazeIcon.position - playerPointerRaycaster.rayTransform.position;
                Ray heldPositionRay = new Ray(transform.position, rayDir);
            ...
            {
                Vector2 currentTouchPos = OVRInput.Get(OVRInput.Axis2D.PrimaryTouchpad);
    ...
    public void Grab()
    {
        ...
        Vector3 rayDir = playerPointerRaycaster.gazeIcon.position - playerPointerRaycaster.rayTransform.position;
        Ray pointerRay = new Ray(playerPointerRaycaster.rayTransform.position, rayDir);
        RaycastHit hit;
        if(Physics.Raycast(pointerRay, out hit, MaxReachDistance, Physics.AllLayers, QueryTriggerInteraction.Ignore))
        {
            if(hit.rigidbody)
            {
